...
expected_errors = 'Article format is not valid \n' \
...
@pytest.mark.parametrize("inventory_file, products_file, expected", [
    ("tests/cli/data/inventory_invalid.json", "tests/cli/data/products_invalid.json", expected_errors),
])
...
expected_text = "All available products, based on current inventory: \n" \
...
@pytest.mark.parametrize("inventory_file, products_file, expected", [
    ("tests/cli/data/inventory.json", "tests/cli/data/products.json", expected_text),
])
def test_cli_with_arg(runner, inventory_file, products_file, expected):
    result = runner.invoke(cli.main, args=["-if", inventory_file, "-pf", products_file], input='2\n')
    assert result.exit_code == 0
    assert not result.exception
    assert result.output.strip() == expected